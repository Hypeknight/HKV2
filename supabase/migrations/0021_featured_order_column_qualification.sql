-- ============================================================================
-- HypeKnight V3.5 - Featured Order Column Qualification
-- ============================================================================
--
-- Fix PL/pgSQL ambiguity between RETURNS TABLE output names such as
-- expires_at/order_id and featured_reservations columns.
--
-- Commerce behavior is unchanged.
-- ============================================================================
...
begin
  ...
  v_discovery_end := coalesce(
    ...
    case
      ...
      else v_event.event_start_at + interval '35 minutes'
    end
  );
  ...
  update public.featured_reservations as fr
  set
    status = 'expired',
    updated_at = now()
  where fr.inventory_id = any(v_inventory_ids)
  ...
  loop
    ...
    select fr.*
      into v_existing
    from public.featured_reservations as fr
    where fr.inventory_id = v_inventory.id
      and fr.event_id = p_event_id
      and fr.user_id = v_user_id
      and fr.status = 'reserved'
      and fr.expires_at > now()
      and fr.order_id is null
