    ...
    <section className="rounded-[2.75rem] border border-accent/20 bg-accent/10 p-10">
      <div className="grid gap-8 lg:grid-cols-[1fr_320px] lg:items-center">
        <div>
          ...
          <h2 className="mt-3 text-4xl font-black text-white">Help move the nightlife network.</h2>
          <p className="mt-4 max-w-3xl text-white/75">
            Ambassadors help bring promoters, DJs, creators, and nightlife communities into HypeKnight.
          </p>

          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Link href="/ambassadors" className="rounded-2xl bg-accent px-6 py-3 text-center font-semibold text-black hover:opacity-90">
              Learn About Ambassadors
            </Link>
            <Link href="/dashboard/ambassador/apply" className="rounded-2xl border border-white/10 bg-black/20 px-6 py-3 text-center text-white hover:border-accent/40">
        ...
        <div className="rounded-[2rem] border border-white/10 bg-black/20 p-8 text-center">
          <p className="text-4xl font-black text-white">{settings.ambassador_min_discount}–{settings.ambassador_max_discount}%</p>
...
function normalizeEvent(event: any, source: 'hypeknight' | 'external') {
  return {
    ...
    source,
    source_label: source === 'hypeknight' ? 'HypeKnight Event' : event.source_code || 'External Event',
...
function isLiveNow(event: any, now: Date) {
  if (!event.event_start_at) return false;
  const start = new Date(event.event_start_at);
  const end = event.event_end_at ? new Date(event.event_end_at) : new Date(start.getTime() + 4 * 60 * 60 * 1000);
...
function FeatureCard({ title, text, href, action }: { title: string; text: string; href: string; action: string }) {
  return (
    <Link href={href} className="group rounded-[2rem] border border-white/10 bg-white/5 p-8 transition hover:border-accent/40 hover:bg-white/[0.07]">
      <h2 className="text-2xl font-bold text-white group-hover:text-accent">{title}</h2>
