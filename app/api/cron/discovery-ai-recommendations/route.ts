...
export async function GET(req: Request) {
  ...
  const cronLog = await startCronLog('discovery-ai-recommendations');
  const supabase = createAdminClient();
  ...
  try {
    ...
    for (const target of targets ?? []) {
      ...
      signals.push({
        ...
        uniqueUsers,
        hypeknightEventCount: hypeEvents?.length ?? 0,
        externalEventCount: externalEvents?.length ?? 0,
        priorityLevel: target.priority_level || 'normal',
        preferredKeywords: target.preferred_keywords || [],
      });
    }
    ...
    if (!signals.length) {
      await completeCronLog({
        ...
        recordsProcessed: 0,
        recordsUpdated: 0,
        notes: 'No enabled discovery targets found.',
      });
    ...
    await completeCronLog({
      ...
      recordsUpdated: created,
      notes: 'Discovery recommendations scanned.',
    });
