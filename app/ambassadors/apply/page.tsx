    ...
    <section className="mx-auto max-w-5xl space-y-10 px-4 py-12 sm:px-6 lg:px-8">
      <div className="rounded-[2.75rem] border border-white/10 bg-white/5 p-8 sm:p-10">
        ...
        <p className="mt-4 max-w-3xl text-white/70">
          Ambassadors help introduce promoters, event hosts, and nightlife communities
          to HypeKnight. Applications are reviewed by HypeKnight before approval.
        </p>

        <div className="mt-6 rounded-2xl border border-accent/20 bg-accent/10 p-5 text-white/75">
          Approved ambassadors may request a personal coupon code between 20% and 70%
          off. Commission is calculated after HypeKnight profit and only becomes eligible
          after the referred event completes its full promotion pipeline with no refund,
          removal, or chargeback.
        </div>
      </div>
      ...
      >
        ...
        <FormSection title="Social Platforms">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Input name="estimated_followers" label="Estimated Total Followers" type="number" />
          </div>
        </FormSection>
        ...
        <label className="flex gap-3 rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-white/70">
          ...
          <span>
            I understand that ambassador approval, coupon activation, commission
            eligibility, and payouts are subject to HypeKnight review.
          </span>
        </label>
