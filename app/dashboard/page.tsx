    ...
    <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
      <div className="grid gap-6 lg:grid-cols-[1fr_0.9fr]">
        <div className="rounded-[2rem] border border-white/10 bg-white/5 p-8">
          ...
          <h1 className="mt-3 text-4xl font-bold text-white">{profile?.display_name || user.email}</h1>
          <p className="mt-3 max-w-2xl text-white/70">
            Manage your account, track your venues, and move into the admin area when you are ready to publish events.
          </p>
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            <div className="rounded-3xl border border-white/10 bg-black/20 p-5">
              <p className="text-xs uppercase tracking-[0.25em] text-white/50">Role</p>
              <p className="mt-3 text-xl font-semibold text-white">{profile?.app_role || 'user'}</p>
            </div>
            <div className="rounded-3xl border border-white/10 bg-black/20 p-5">
