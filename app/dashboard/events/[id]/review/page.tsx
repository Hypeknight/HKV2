...
type Props = { params: Promise<{id:string}>; searchParams?: Promise<{submitted?:string; paid?:string}> };
...
export default async function EventReviewPage({ params, searchParams }: Props) {
  ...
  return <section className="mx-auto max-w-6xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
    ...
    {query.submitted ? <Notice>Event submitted successfully.</Notice> : null}{query.paid ? <Notice>Payment reconciled successfully.</Notice> : null}
    <div className="grid gap-6 lg:grid-cols-[1.2fr_.8fr]">
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-[2rem] border border-white/10 bg-white/5 p-6"><h2 className="text-xl font-black text-white">Ready?</h2><p className="mt-2 text-sm leading-6 text-white/50">Submitting preserves the existing HypeKnight moderation workflow. If payment is still due, you will be routed to checkout first.</p><form action={submitEventForModeration} className="mt-5"><input type="hidden" name="event_id" value={id}/><button className="w-full rounded-2xl bg-white px-5 py-4 font-black text-black">Submit event</button></form>{['building','draft','rejected'].includes(event.status) ? <form action={discardDraftEvent} className="mt-3"><input type="hidden" name="event_id" value={id}/><button className="w-full rounded-2xl border border-red-500/20 px-5 py-3 text-sm font-bold text-red-200">Discard draft</button></form>:null}</section>
      </div>
    </div>
  </section>;
}
