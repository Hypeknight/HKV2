    ...
    <section className="mx-auto max-w-3xl space-y-6 px-4 py-6 sm:px-6 sm:py-10">
      ...
      {existingClaim && existingClaim.status === 'pending' ? (
        ...
        <form action={submitExternalEventClaim} className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-5 sm:p-7">
          ...
          {!(ownedEvents || []).length ? <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4"><p className="font-bold text-white">No HypeKnight events yet?</p><p className="mt-1 text-sm leading-6 text-white/50">You can submit the claim now without a connection. When you create your HypeKnight listing later, the claim can be linked during review.</p><Link href="/dashboard/events/new/step-1" className="mt-3 inline-flex text-sm font-black text-accent">Create a HypeKnight event →</Link></div> : null}
